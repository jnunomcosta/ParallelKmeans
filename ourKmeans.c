    ...
    while(execution<max_executions){
        ...
        for(;mudou;iteracoes++){
            ...
            for(int i=0;i<n;i++){
                ...
                clusters[cluster_closest_to][clusters_size[cluster_closest_to]++]=points[i];
            }
            int mudou_ou_nao = 1;
            ...
            for(int i=0;i<k;i++){
                ...
                if(!(x==0 && y==0)){
                    ...
                    if(centroids_anteriores[i].x!=centroids[i].x || centroids_anteriores[i].y!=centroids[i].y){
                        mudou_ou_nao = 0;
                    }
                }
            }
        ...
        printf("Media = %f segundos ; execucao = %d\n",auxiasda,execution);
